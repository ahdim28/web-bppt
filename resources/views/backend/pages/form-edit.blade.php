...
<div class="card">
    <div class="list-group list-group-flush account-settings-links flex-row">
        ...
        @if ($data['fields']->count() > 0 || !empty($data['page']->field_category_id) || Auth::user()->hasRole('super'))
    ...
    <form action="{{ route('page.update', ['id' => $data['page']->id]) }}" method="POST">
        ...
        <div class="tab-content">
                ...
                <div class="card-body overflow-hidden">
                    <div class="row no-gutters row-bordered row-border-light">
                        ...
                        <div class="col-md-{{ config('custom.language.multiple') == true ? '9' : '12' }}">
                            <div class="tab-content">
                                <input type="hidden" name="url_id" value="{{ $data['page']->indexing->id }}">
                                @foreach ($data['languages'] as $lang)
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-10">
                                                <input type="text" class="form-control mb-1 gen_slug @error('title_'.$lang->iso_codes) is-invalid @enderror" lang="{{ $lang->iso_codes }}" 
                                                    name="title_{{ $lang->iso_codes }}" value="{{ old('title_'.$lang->iso_codes, $data['page']->fieldLang('title', $lang->iso_codes)) }}" placeholder="Enter title...">
            ...
            <div class="tab-pane fade" id="setting">
                <div class="card-body">
                        ...
                        <div class="col-md-10">
                            <label class="switcher switcher-success">
                                <input type="checkbox" class="switcher-input check-parent" name="public" value="1" 
                                    {{ (old('public', $data['page']->public) == 1 ? 'checked' : '') }}>
                        ...
                        <div class="col-md-10">
                            <label class="switcher switcher-success">
                                <input type="checkbox" class="switcher-input check-parent" name="is_detail" value="1" 
                                    {{ (old('is_detail', $data['page']->is_detail) == 1 ? 'checked' : '') }}>
                                <span class="switcher-indicator">
                                  <span class="switcher-yes">
                                    <span class="ion ion-md-checkmark"></span>
                                  </span>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input class="form-control mb-1" name="meta_keywords" id="bs-tagsinput-2" value="{{ old('meta_keywords', $data['page']->meta_data['keywords']) }}" placeholder="">
            ...
            @if (!empty($data['page']->field_category_id) || Auth::user()->hasRole('super'))
            <div class="tab-pane fade" id="field">
                <div class="card-body">
                        ...
                        <div class="col-sm-10">
                            <select id="field_category" class="selectpicker show-tick" name="field_category_id" data-style="btn-default">
                                ...
                                @foreach ($data['fields'] as $field)
                                    <option value="{{ $field->id }}" {{ $field->id == $data['page']->field_category_id ? 'selected' : '' }}>
                                        {{ $field->name }}
                                    </option>
                                @endforeach
                            </select>
                            @if (!empty($data['page']->field_category_id))    
                            <small>Current Field : <strong><i>{{ $data['page']->field->name }}</i></strong></small>
                            @endif
                        </div>
                    </div>
                    <div id="custom_field">
                    ...
                    @else
                    <input type="hidden" name="field_category_id" value="{{ $data['page']->field_category_id }}">
                    @endrole

                    @if (!empty($data['page']->field_category_id))
                    <div id="current_field">
                        @include('backend.master.fields.input-edit', ['module' => 'page'])
                    </div>
                    @endif
                </div>
            </div>
