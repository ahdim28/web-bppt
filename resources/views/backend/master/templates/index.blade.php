...
<div class="card">
    <div class="card-body">
        <div class="form-row align-items-center">
            ...
            <div class="col-md">
                <div class="form-group">
                    <label class="form-label">@lang('mod/master.template.label.field2')</label>
                    <select class="module custom-select" name="m">
                        ...
                        <option value="{{ $key }}" {{ Request::get('m') == ''.$key.'' ? 'selected' : '' }} title="Filter by 
                            {{ str_replace('_', ' ', ucfirst($val)) }}">{{ str_replace('_', ' ', ucfirst($val)) }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
...
<div class="card">
    <div class="card-header with-elements">
        <h5 class="card-header-title mt-1 mb-0">@lang('mod/master.template.text')</h5>
        <div class="card-header-elements ml-auto">
            <a href="{{ route('template.create') }}" class="btn btn-success icon-btn-only-sm" title="@lang('lang.add_attr_new', [
                'attribute' => __('mod/master.template.caption')
            ])">
                <i class="las la-plus"></i> <span>@lang('mod/master.template.caption')</span>
            </a>
        </div>
    </div>
        ...
        <table id="user-list" class="table card-table table-striped table-bordered table-hover">
            <thead>
                <tr>
                    ...
                    <th>@lang('mod/master.template.label.field1')</</th>
                    <th class="text-center">@lang('mod/master.template.label.field2')</th>
                    <th>@lang('mod/master.template.label.field3')</th>
                    <th>@lang('mod/master.template.label.field4')</th>
                    <th style="width: 215px;">@lang('lang.created')</th>
                    <th style="width: 215px;">@lang('lang.updated')</th>
                    <th class="text-center" style="width: 115px;">@lang('lang.action')</th>
                </tr>
            </thead>
            <tbody>
                ...
                @foreach ($data['templates'] as $item)
                    <tr>
                        ...
                        <td class="text-center"><span class="badge badge-primary">{{ Str::replace('_', ' ', Str::upper($item->customConfig()['module'])) }}</span></td>
                        <td>{{ $item->customConfig()['type'] }}</td>
                        ...
                        <td class="text-center">
                            <a href="{{ route('template.edit', ['id' => $item->id]) }}" class="btn btn-primary icon-btn btn-sm" title="@lang('lang.edit_attr', [
                                'attribute' => __('mod/master.template.caption')
                            ])">
                            ...
                            <div class="card">
                                <div class="card-body">
                                    <div class="item-table">
                                        <div class="data-table">@lang('mod/master.template.label.field1')</div>
                                        <div class="desc-table"><strong>{{ $item->name }}</strong></div>
                                    </div>
                                    <div class="item-table">
                                        <div class="data-table">@lang('mod/master.template.label.field2')</div>
                                        <div class="desc-table"><span class="badge badge-primary">{{ Str::replace('_', ' ', Str::upper($item->customConfig()['module'])) }}</span></div>
                                    </div>
                                    <div class="item-table">
                                        <div class="data-table">@lang('mod/master.template.label.field3')</div>
                                        <div class="desc-table">{{ $item->customConfig()['type'] }}</div>
                                    </div>
                                    <div class="item-table">
                                        <div class="data-table">@lang('mod/master.template.label.field4')</div>
                                        <div class="desc-table"><code>{{ $item->file_path }}</code></div>
                                    </div>
                                    <div class="item-table">
        ...
        $('.swal-delete').on('click', function () {
            ...
            Swal.fire({
                ...
                if (response.value.success) {
                    Swal.fire({
                        type: 'success',
                        text: "@lang('alert.delete_success', ['attribute' => __('mod/master.template.caption')])"
                    }).then(() => {
