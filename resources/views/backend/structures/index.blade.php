...
<div class="card">
    <div class="card-header with-elements">
        <h5 class="card-header-title mt-1 mb-0">Structure Organization List</h5>
        <div class="card-header-elements ml-auto">
            @can ('structure_create')
            <a href="{{ route('structure.create') }}" class="btn btn-success icon-btn-only-sm" title="Add New Structure Organization">
                <i class="las la-plus"></i> <span>Structure Organization</span>
            </a>
            @endcan
        </div>
    </div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center">
                        @if (auth()->user()->can('structure_update') && $item->min('position') != $item->position)
                        <a href="javascript:void(0);" onclick="$(this).find('form').submit();" class="btn icon-btn btn-sm btn-dark" data-toggle="tooltip" data-original-title="@lang('lang.position') Up">
                            ...
                            <form action="{{ route('structure.position', ['id' => $item->id, 'position' => ($item->position - 1)]) }}" method="POST">
                                @csrf
                                @method('PUT')
                            </form>
                        </a>
                        ...
                        @endif
                        @if (auth()->user()->can('structure_update') && $item->max('position') != $item->position)
                    ...
                    <td class="text-center">
                        <a href="{{ route('structure.read', ['slugStructure' => $item->slug]) }}" class="btn icon-btn btn-sm btn-info" title="View Detail" target="_blank">
                            <i class="las la-external-link-alt"></i>
                        </a>
                        @can('structure_update')
                        <a href="{{ route('structure.edit', ['id' => $item->id]) }}" class="btn icon-btn btn-sm btn-primary" title="Edit Structure Organization">
                            <i class="las la-pen"></i>
                        </a>
                        @endcan
                        @can('structure_delete')
                        <a href="javascript:;" data-id="{{ $item->id }}" class="btn icon-btn btn-sm btn-danger swal-delete" title="Delete Structure Organization">
    ...
    <div class="card-footer">
        <div class="row align-items-center">
            <div class="col-lg-6 m--valign-middle">
                @lang('pagination.showing') : <strong>{{ $data['structures']->firstItem() }}</strong> - <strong>{{ $data['structures']->lastItem() }}</strong> @lang('pagination.of')
                <strong>{{ $data['structures']->total() }}</strong>
            </div>
            <div class="col-lg-6 m--align-right">
                {{ $data['structures']->onEachSide(1)->links() }}
            </div>
        </div>
    </div>
</div>
        ...
        $('.swal-delete').on('click', function () {
            ...
            Swal.fire({
                ...
                if (response.value.success) {
                    Swal.fire({
                        type: 'success',
                        text: "@lang('alert.delete_success', ['attribute' => 'Structure Organization'])"
                    }).then(() => {
