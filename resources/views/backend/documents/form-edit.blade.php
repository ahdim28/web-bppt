...
<div class="card">
    ...
    <form action="{{ route('document.update', ['categoryId' => $data['category']->id, 'id' => $data['document']->id]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                                    ...
                                    <div class="card-body pb-2">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-10">
                                                <input type="text" class="form-control mb-1 gen_slug @error('title_'.$lang->iso_codes) is-invalid @enderror" lang="{{ $lang->iso_codes }}" 
                                                    name="title_{{ $lang->iso_codes }}" value="{{ old('title_'.$lang->iso_codes, $data['document']->fieldLang('title', $lang->iso_codes)) }}" placeholder="Enter name...">
                                            ...
                                            <div class="col-sm-10">
                                                <div class="input-group">
                                                    ...
                                                    <input type="text" class="form-control slug_spot @error('slug') is-invalid @enderror" lang="{{ $lang->iso_codes }}" name="slug"
                                                        value="{{ old('slug', $data['document']->slug) }}" placeholder="Enter slug...">
                                                    @include('components.field-error', ['field' => 'slug'])
                                                </div>
                                            </div>
                                        </div>
                                        @if ($data['document']->from == 0)
                                        <div class="form-group row" id="file">
                                            ...
                                            <div class="col-md-10">
                                                ...
                                                <input type="hidden" name="old_file" value="{{ $data['document']->file }}">
                                            ...
                                            <div class="col-sm-10">
                                                <div class="input-group">
                                                    <input type="text" class="form-control @error('document_url') is-invalid @enderror" id="image1" aria-label="Image" aria-describedby="button-image" name="document_url"
                                                            value="{{ old('document_url', $data['document']->document_url) }}" placeholder="browse file...">
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-10">
                                                <textarea class="form-control tiny-mce" name="description_{{ $lang->iso_codes }}">{!! old('description_'.$lang->iso_codes, $data['document']->fieldLang('description', $lang->iso_codes)) !!}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="col-sm-10">
                            <select class="selectpicker show-tick" name="publish" data-style="btn-default">
                                @foreach (config('custom.label.publish') as $key => $publish)
                                    <option value="{{ $key }}" {{ (old('publish', $data['document']->publish) == ''.$key.'') ? 'selected' : '' }}>
                                        {{ __($publish['title']) }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-10">
                            <label class="switcher switcher-success">
                                <input type="checkbox" class="switcher-input check-parent" name="public" value="1" 
                                    {{ (old('public', $data['document']->public) == 1 ? 'checked' : 'checked') }}>
                        ...
                        <div class="col-sm-10">
                            <div class="input-group">
                                <input type="text" class="form-control" id="image2" aria-label="Image2" aria-describedby="button-image2" name="cover_file"
                                        value="{{ old('cover_file', $data['document']->cover['file_path']) }}" placeholder="browse cover..." readonly>
                                <div class="input-group-append" title="browse file">
                            ...
                            <div class="row">
                                <div class="col-sm-6">
                                <input type="text" class="form-control" placeholder="title..." name="cover_title" value="{{ old('cover_title', $data['document']->cover['title']) }}">
                                </div>
                                <div class="col-sm-6">
                                <input type="text" class="form-control" placeholder="alt..." name="cover_alt" value="{{ old('cover_alt', $data['document']->cover['alt']) }}">
                                </div>
                            </div>
                        </div>
                    </div>
