...
<div class="card">
    <div class="list-group list-group-flush account-settings-links flex-row">
        ...
        @if ($data['fields']->count() > 0 || !empty($data['section']->field_category_id) || Auth::user()->hasRole('super'))
        <a class="list-group-item list-group-item-action" data-toggle="list" href="#field">CUSTOM FIELD</a>
        @endif
    </div>
    <form action="{{ route('section.update', ['id' => $data['section']->id]) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="tab-content">
                ...
                <div class="card-body overflow-hidden">
                    <div class="row no-gutters row-bordered row-border-light">
                        ...
                        <div class="col-md-{{ config('custom.language.multiple') == true ? '9' : '12' }}">
                            <div class="tab-content">
                                <input type="hidden" name="url_id" value="{{ $data['section']->indexing->id }}">
                                @foreach ($data['languages'] as $lang)
                                <div class="tab-pane fade {{ $lang->iso_codes == config('custom.language.default') ? 'active show' : '' }}" id="{{ $lang->iso_codes }}">
                                    <div class="card-body pb-2">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-10">
                                                <input type="text" class="form-control mb-1 gen_slug @error('name_'.$lang->iso_codes) is-invalid @enderror" lang="{{ $lang->iso_codes }}" 
                                                    name="name_{{ $lang->iso_codes }}" value="{{ old('name_'.$lang->iso_codes, $data['section']->fieldLang('name', $lang->iso_codes)) }}" placeholder="Enter name...">
                                            ...
                                            <div class="col-sm-10">
                                                <div class="input-group">
                                                    ...
                                                    <input type="text" class="form-control slug_spot @error('slug') is-invalid @enderror" lang="{{ $lang->iso_codes }}" name="slug"
                                                        value="{{ old('slug', $data['section']->slug) }}" placeholder="Enter slug...">
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-10">
                                                <textarea class="form-control tiny-mce" name="description_{{ $lang->iso_codes }}">{!! old('description_'.$lang->iso_codes, $data['section']->fieldLang('description', $lang->iso_codes)) !!}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                </div>
            ...
            <div class="tab-pane fade" id="setting">
                <div class="card-body">
                        ...
                        <div class="col-sm-10">
                            <select class="custom-select" name="extra" data-style="btn-default">
                                @foreach (config('custom.label.extra') as $key => $extra)
                                    <option value="{{ $key }}" {{ (old('extra', $data['section']->extra) == ''.$key.'') ? 'selected' : '' }}>
                                        {{ $extra['title'] }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-sm-10">
                            <select class="custom-select" name="order_field" data-style="btn-default">
                                @foreach (config('custom.filtering.ordering_field_posts') as $key => $value)
                                    <option value="{{ $value }}" {{ (old('order_field', $data['section']->order_field) == $value) ? 'selected' : '' }}>
                                        {{ $value }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-sm-10">
                            <select class="custom-select" name="order_by" data-style="btn-default">
                                @foreach (config('custom.filtering.ordering') as $key => $ordering)
                                    <option value="{{ $ordering['title'] }}" {{ (old('order_by', $data['section']->order_by) == $ordering['title']) ? 'selected' : '' }}>
                        ...
                        <div class="col-md-10">
                            <label class="switcher switcher-success">
                                <input type="checkbox" class="switcher-input check-parent" name="public" value="1" 
                                    {{ (old('public', $data['section']->public) == 1 ? 'checked' : '') }}>
                        ...
                        <div class="col-md-10">
                            <label class="switcher switcher-success">
                                <input type="checkbox" class="switcher-input check-parent" name="is_detail" value="1" 
                                    {{ (old('is_detail', $data['section']->is_detail) == 1 ? 'checked' : '') }}>
                                <span class="switcher-indicator">
                                  <span class="switcher-yes">
                                    <span class="ion ion-md-checkmark"></span>
                                  </span>
                        ...
                        <div class="col-sm-10">
                            <div class="input-group">
                                <input type="text" class="form-control" id="image1" aria-label="Image" aria-describedby="button-image" name="banner_file"
                                        value="{{ old('banner_file', $data['section']->banner['file_path']) }}" placeholder="browse banner..." readonly>
                            ...
                            <div class="row">
                                <div class="col-sm-6">
                                <input type="text" class="form-control" placeholder="title..." name="banner_title" value="{{ old('banner_title', $data['section']->banner['title']) }}">
                                </div>
                                <div class="col-sm-6">
                                <input type="text" class="form-control" placeholder="alt..." name="banner_alt" value="{{ old('banner_alt', $data['section']->banner['alt']) }}">
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-10">
                            <select class="selectpicker show-tick" name="list_view_id" data-style="btn-default">
                                ...
                                @foreach ($data['template_list'] as $custom)
                                    <option value="{{ $custom->id }}" {{ (old('list_view_id', $data['section']->list_view_id) == $custom->id) ? 'selected' : '' }}>
                                        {{ $custom->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-sm-10">
                            <select class="selectpicker show-tick" name="detail_view_id" data-style="btn-default">
                                ...
                                @foreach ($data['template_detail'] as $custom)
                                    <option value="{{ $custom->id }}" {{ (old('detail_view_id', $data['section']->detail_view_id) == $custom->id) ? 'selected' : '' }}>
                                        {{ $custom->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="number" class="form-control" name="limit_category" value="{{ old('limit_category', $data['section']->limit_category) }}" placeholder="for category">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-form-label col-sm-2 text-sm-right">Content Limit <i>(post)</i></label>
                        <div class="col-sm-10">
                            <input type="number" class="form-control" name="limit_post" value="{{ old('limit_post', $data['section']->limit_post) }}" placeholder="for post">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-form-label col-sm-2 text-sm-right">Post Selection</label>
                        <div class="col-sm-10">
                            <input type="number" class="form-control" name="post_selection" value="{{ old('post_selection', $data['section']->post_selection) }}" placeholder="for post">
                        </div>
                    </div>
                </div>
            </div>

            @if (!empty($data['section']->field_category_id) || Auth::user()->hasRole('super'))
            <div class="tab-pane fade" id="field">
                <div class="card-body">
                        ...
                        <div class="col-sm-10">
                            <select id="field_category" class="selectpicker show-tick" name="field_category_id" data-style="btn-default">
                                ...
                                @foreach ($data['fields'] as $field)
                                    <option value="{{ $field->id }}" {{ $field->id == $data['section']->field_category_id ? 'selected' : '' }}>
                                        {{ $field->name }}
                                    </option>
                                @endforeach
                            </select>
                            @if (!empty($data['section']->field_category_id))    
                            <small>Current Field : <strong><i>{{ $data['section']->field->name }}</i></strong></small>
                            @endif
                        </div>
                    </div>
                    <div id="custom_field">
                    ...
                    @else
                    <input type="hidden" name="field_category_id" value="{{ $data['section']->field_category_id }}">
                    @endrole

                    @if (!empty($data['section']->field_category_id))
                    <div id="current_field">
                        @include('backend.master.fields.input-edit', ['module' => 'section'])
                    </div>
                    @endif
                </div>
            </div>
