...
<div class="card">
    ...
    <form action="{{ route('category.update', ['sectionId' => $data['section']->id, 'id' => $data['category']->id]) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="tab-content">
                                            ...
                                            <div class="col-sm-10">
                                                <div class="input-group">
                                                    <div class="input-group-prepend">
                                                        <span class="input-group-text"><strong>{{ url('/').'/'.$data['section']->slug.'/cat/' }}</strong></span>
                                                    </div>
            ...
            <div class="tab-pane fade" id="setting">
                <div class="card-body">
                        ...
                        <div class="col-sm-10">
                            <select class="selectpicker show-tick" name="publish" data-style="btn-default">
                                @foreach (config('custom.label.publish') as $key => $publish)
                                    <option value="{{ $key }}" {{ (old('publish', $data['category']->publish) == ''.$key.'') ? 'selected' : '' }}>
                                        {{ __($publish['title']) }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-10">
                            <label class="switcher switcher-success">
                                <input type="checkbox" class="switcher-input check-parent" name="public" value="1" 
                                    {{ (old('public', $data['category']->public) == 1 ? 'checked' : '') }}>
                        ...
                        <div class="col-sm-10">
                            <select class="selectpicker show-tick" name="list_view_id" data-style="btn-default">
                                ...
                                @foreach ($data['template_list'] as $custom)
                                    <option value="{{ $custom->id }}" {{ (old('list_view_id', $data['category']->list_view_id) == $custom->id) ? 'selected' : '' }}>
                                        {{ $custom->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-sm-10">
                            <select class="selectpicker show-tick" name="detail_view_id" data-style="btn-default">
                                ...
                                @foreach ($data['template_detail'] as $custom)
                                    <option value="{{ $custom->id }}" {{ (old('detail_view_id', $data['category']->detail_view_id) == $custom->id) ? 'selected' : '' }}>
                                        {{ $custom->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <hr>
                    <div class="form-group row">
                        <label class="col-form-label col-sm-2 text-sm-right">List Limit <i>(post)</i></label>
                        <div class="col-sm-10">
                            <input type="number" class="form-control" name="list_limit" value="{{ old('list_limit', $data['category']->list_limit) }}" placeholder="for post">
                        </div>
                    </div>
                </div>
            </div>
