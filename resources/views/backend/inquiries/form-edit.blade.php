...
<div class="card">
    <div class="list-group list-group-flush account-settings-links flex-row">
        ...
        @if ($data['fields']->count() > 0 || !empty($data['inquiry']->field_category_id) || auth()->user()->hasRole('super'))
        ...
        <div class="tab-content">
                ...
                <div class="card-body overflow-hidden">
                    <div class="row no-gutters row-bordered row-border-light">
                        ...
                        <div class="col-md-{{ config('custom.language.multiple') == true ? '9' : '12' }}">
                            <div class="tab-content">
                                <input type="hidden" name="url_id" value="{{ $data['inquiry']->indexing->id }}">
                                @foreach ($data['languages'] as $lang)
                                <div class="tab-pane fade {{ $lang->iso_codes == config('custom.language.default') ? 'active show' : '' }}" id="{{ $lang->iso_codes }}">
                                    <div class="card-body pb-2">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-10">
                                                <input type="text" class="form-control mb-1 gen_slug @error('name_'.$lang->iso_codes) is-invalid @enderror" lang="{{ $lang->iso_codes }}" 
                                                    name="name_{{ $lang->iso_codes }}" value="{{ old('name_'.$lang->iso_codes, $data['inquiry']->fieldLang('name', $lang->iso_codes)) }}" placeholder="Enter name...">
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-10">
                                                <textarea class="form-control tiny-mce @error('body_'.$lang->iso_codes) is-invalid @enderror" name="body_{{ $lang->iso_codes }}">{!! old('body_'.$lang->iso_codes, $data['inquiry']->fieldLang('body', $lang->iso_codes)) !!}</textarea>
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-10">
                                                <textarea class="form-control tiny-mce" name="after_body_{{ $lang->iso_codes }}">{!! old('after_body_'.$lang->iso_codes, $data['inquiry']->fieldLang('after_body', $lang->iso_codes)) !!}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                </div>
            ...
            <div class="tab-pane fade" id="setting">
                <div class="card-body">
                        ...
                        <div class="col-md-10">
                            <label class="switcher switcher-success">
                                <input type="checkbox" class="switcher-input check-parent" name="is_detail" value="1" 
                                    {{ (old('is_detail', $data['inquiry']->is_detail) == 1 ? 'checked' : '') }}>
                    ...
                    <div class="form-group row">
                        <div class="col-md-2 text-md-right">
                          <label class="col-form-label text-sm-right">Show Form</label>
                        </div>
                        <div class="col-md-10">
                            <label class="switcher switcher-success">
                                <input type="checkbox" class="switcher-input check-parent" name="show_form" value="1" 
                                    {{ (old('show_form', $data['inquiry']->show_form) == 1 ? 'checked' : '') }}>
                                <span class="switcher-indicator">
                                  <span class="switcher-yes">
                                    <span class="ion ion-md-checkmark"></span>
                                  </span>
                    ...
                    <div class="form-group row">
                        <label class="col-form-label col-sm-2 text-sm-right">Email <i>(for notifications)</i></label>
                        <div class="col-sm-10">
                            <input type="text" value="{{ !empty($data['inquiry']->email) ? old('email', $data['inquiry']->mailStr()['implode']) : old('email') }}" name="email" class="form-control" id="bs-tagsinput-2">
                            <small><i>*Separated by comma (,). Example : <strong>admin@gmail.com, guest@gmail.com,</strong></i></small>
                        </div>
                    </div>
                    <div class="form-group row">
                        <div class="col-md-2 text-md-right">
                          <label class="col-form-label text-sm-right">Show Map</label>
                        </div>
                        <div class="col-md-10">
                            <label class="switcher switcher-success">
                                <input type="checkbox" class="switcher-input check-parent" name="show_map" value="1" 
                                    {{ (old('show_map', $data['inquiry']->show_map) == 1 ? 'checked' : '') }}>
                                <span class="switcher-indicator">
                                  <span class="switcher-yes">
                                    <span class="ion ion-md-checkmark"></span>
                                  </span>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control mb-1" name="meta_title" value="{{ old('meta_title', $data['inquiry']->meta_data['title']) }}" placeholder="Enter meta title...">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-form-label col-sm-2 text-sm-right">Meta Description</label>
                        <div class="col-sm-10">
                            <textarea class="form-control mb-1" name="meta_description" placeholder="Separated by comma (,)">{{ old('meta_decsription', $data['inquiry']->meta_data['description']) }}</textarea>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-form-label col-sm-2 text-sm-right">Meta Keywords</label>
                        <div class="col-sm-10">
                            <input class="form-control mb-1" name="meta_keywords" id="bs-tagsinput-3" value="{{ old('meta_keywords', $data['inquiry']->meta_data['keywords']) }}" placeholder="">
            ...
            @if (!empty($data['inquiry']->field_category_id) || auth()->user()->hasRole('super'))
            <div class="tab-pane fade" id="field">
                <div class="card-body">
                        ...
                        <div class="col-sm-10">
                            <select id="field_category" class="selectpicker show-tick" name="field_category_id" data-style="btn-default">
                                ...
                                @foreach ($data['fields'] as $field)
                                    <option value="{{ $field->id }}" {{ $field->id == $data['inquiry']->field_category_id ? 'selected' : '' }}>
                                        {{ $field->name }}
                                    </option>
                                @endforeach
                            </select>
                            @if (!empty($data['inquiry']->field_category_id))    
                            <small>Current Field : <strong><i>{{ $data['inquiry']->field->name }}</i></strong></small>
                            @endif
                        </div>
                    </div>
                    <div id="custom_field">
                    ...
                    @else
                    <input type="hidden" name="field_category_id" value="{{ $data['inquiry']->field_category_id }}">
                    @endrole

                    @if (!empty($data['inquiry']->field_category_id))
                    <div id="current_field">
                        @include('backend.master.fields.input-edit', ['module' => 'inquiry'])
                    </div>
                    @endif
                </div>
            </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        var show_map = '{{ $data['inquiry']->show_map }}';
        if (show_map == 1) {
            $('#long-lat').show();
        } else {
            $('#long-lat').hide();
