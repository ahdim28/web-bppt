...
<div class="card">
    <div class="list-group list-group-flush account-settings-links flex-row">
        ...
        @if ($data['fields']->count() > 0 || !empty($data['media']->field_category_id) || auth()->user()->hasRole('super'))
    ...
    <form action="{{ route('link.media.update', ['linkId' => $data['link']->id, 'id' => $data['media']->id]) }}" method="POST">
        ...
        <div class="tab-content">
                                ...
                                <div class="tab-pane fade {{ $lang->iso_codes == config('custom.language.default') ? 'active show' : '' }}" id="{{ $lang->iso_codes }}">
                                    <div class="card-body pb-2">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-10">
                                                <input type="text" class="form-control mb-1 @error('title_'.$lang->iso_codes) is-invalid @enderror" lang="{{ $lang->iso_codes }}" 
                                                    name="title_{{ $lang->iso_codes }}" value="{{ old('title_'.$lang->iso_codes, $data['media']->fieldLang('title', $lang->iso_codes)) }}" placeholder="Enter title...">
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-10">
                                                <textarea class="form-control tiny-mce" name="description_{{ $lang->iso_codes }}">{!! old('description_'.$lang->iso_codes, $data['media']->fieldLang('description', $lang->iso_codes)) !!}</textarea>
                                            </div>
                                        </div>
            ...
            @if (!empty($data['media']->field_category_id) || auth()->user()->hasRole('super'))
            <div class="tab-pane fade" id="field">
                <div class="card-body">
                        ...
                        <div class="col-sm-10">
                            <select id="field_category" class="selectpicker show-tick" name="field_category_id" data-style="btn-default">
                                ...
                                @foreach ($data['fields'] as $field)
                                    <option value="{{ $field->id }}" {{ $field->id == $data['media']->field_category_id ? 'selected' : '' }}>
                                        {{ $field->name }}
                                    </option>
                                @endforeach
                            </select>
                            @if (!empty($data['media']->field_category_id))    
                            <small>Current Field : <strong><i>{{ $data['media']->field->name }}</i></strong></small>
                            @endif
                        </div>
                    </div>
                    <div id="custom_field">
                    ...
                    @else
                    <input type="hidden" name="field_category_id" value="{{ $data['media']->field_category_id }}">
                    @endrole

                    @if (!empty($data['media']->field_category_id))
                    <div id="current_field">
                        @include('backend.master.fields.input-edit', ['module' => 'media'])
                    </div>
                    @endif
                </div>
            </div>
