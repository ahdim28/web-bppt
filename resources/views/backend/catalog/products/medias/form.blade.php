...
<div class="row justify-content-center">
    <div class="card mb-4 col-md-10">
        ...
        <form action="{{ route('catalog.product.media.store', ['productId' => $data['product']->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="card-body">
                ...
                <div class="form-group row" id="file">
                    ...
                    <div class="col-md-10">
                        ...
                        <span class="text-muted">
                            File Type : <strong>{{ config('custom.files.product.mimes').','.config('custom.files.product.mimes_video') }}</strong>, 
                            Pixel : <strong>{{ config('custom.files.product.pixel') }}</strong>,
                            Max Size : <strong>{{ config('custom.files.product.size') }} Kilobyte</strong>
                        </span>
                    </div>
                </div>
                <div class="form-group row" id="thumbnail">
                    ...
                    <div class="col-md-10">
                        ...
                        <span class="text-muted">
                            File Type : <strong>{{ config('custom.files.product.thumbnail.mimes').','.config('custom.files.product.mimes_video') }}</strong>, 
                            Pixel : <strong>{{ config('custom.files.product.thumbnail.pixel') }}</strong>,
                            Max Size : <strong>{{ config('custom.files.product.thumbnail.size') }} Kilobyte</strong>
                        </span>
                    </div>
                </div>
                <div class="form-group row">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <textarea class="form-control tiny-mce" name="description">{!! old('description') !!}</textarea>
                    </div>
                </div>
                <div class="form-group row">
