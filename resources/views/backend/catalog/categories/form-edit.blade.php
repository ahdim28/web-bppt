...
<div class="card">
    <div class="list-group list-group-flush account-settings-links flex-row">
        ...
        @if ($data['fields']->count() || !empty($data['category']->field_category_id) || Auth::user()->hasRole('super'))
        <a class="list-group-item list-group-item-action" data-toggle="list" href="#field">CUSTOM FIELD</a>
        @endif
    </div>
    <form action="{{ route('catalog.category.update', ['id' => $data['category']->id]) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="tab-content">
                                            ...
                                            <div class="col-sm-10">
                                                <div class="input-group">
                                                    <div class="input-group-prepend">
                                                        <span class="input-group-text"><strong>{{ url('/').'/catalog/cat/' }}</strong></span>
                                                    </div>
            ...
            <div class="tab-pane fade" id="setting">
                <div class="card-body">
                        ...
                        <div class="col-md-10">
                            <label class="switcher switcher-success">
                                <input type="checkbox" class="switcher-input check-parent" name="is_detail" value="1" 
                                    {{ (old('is_detail', $data['category']->is_detail) == 1 ? 'checked' : '') }}>
                        ...
                        <div class="col-sm-10">
                            <div class="input-group">
                                <input type="text" class="form-control" id="image1" aria-label="Image" aria-describedby="button-image" name="cover_file"
                                        value="{{ old('cover_file', $data['category']->cover['file_path']) }}" placeholder="browse cover..." readonly>
                            ...
                            <div class="row">
                                <div class="col-sm-6">
                                <input type="text" class="form-control" placeholder="title..." name="cover_title" value="{{ old('cover_title', $data['category']->cover['title']) }}">
                                </div>
                                <div class="col-sm-6">
                                <input type="text" class="form-control" placeholder="alt..." name="cover_alt" value="{{ old('cover_alt', $data['category']->cover['alt']) }}">
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-10">
                            <select class="selectpicker show-tick" name="custom_view_id" data-style="btn-default">
                                ...
                                @foreach ($data['template'] as $custom)
                                    <option value="{{ $custom->id }}" {{ (old('custom_view_id', $data['category']->custom_view_id) == $custom->id) ? 'selected' : '' }}>
                                        {{ $custom->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <hr>
                    <div class="form-group row">
                        <label class="col-form-label col-sm-2 text-sm-right">Content Limit <i>(product)</i></label>
                        <div class="col-sm-10">
                            <input type="number" class="form-control" name="product_limit" value="{{ old('product_limit', $data['category']->product_limit) }}" placeholder="for product">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-form-label col-sm-2 text-sm-right">Product Selection</label>
                        <div class="col-sm-10">
                            <input type="number" class="form-control" name="product_selection" value="{{ old('product_selection', $data['category']->product_selection) }}" placeholder="for product">
                        </div>
                    </div>
                    <hr>
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control mb-1" name="meta_title" value="{{ old('meta_title', $data['category']->meta_data['title']) }}" placeholder="Enter meta title...">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-form-label col-sm-2 text-sm-right">Meta Description</label>
                        <div class="col-sm-10">
                            <textarea class="form-control mb-1" name="meta_description" placeholder="Separated by comma (,)">{{ old('meta_decsription', $data['category']->meta_data['description']) }}</textarea>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-form-label col-sm-2 text-sm-right">Meta Keywords</label>
                        <div class="col-sm-10">
                            <input class="form-control mb-1" name="meta_keywords" id="bs-tagsinput-2" value="{{ old('meta_keywords', $data['category']->meta_data['keywords']) }}" placeholder="">
