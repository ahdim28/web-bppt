...
<div class="card">
    <div class="list-group list-group-flush account-settings-links flex-row">
        ...
        @if ($data['fields']->count() > 0 || !empty($data['album']->field_category_id) || auth()->user()->hasRole('super'))
        <a class="list-group-item list-group-item-action" data-toggle="list" href="#field">CUSTOM FIELD</a>
        @endif
    </div>
    <form action="{{ route('gallery.album.update', ['id' => $data['album']->id]) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="tab-content">
                                ...
                                <div class="tab-pane fade {{ $lang->iso_codes == config('custom.language.default') ? 'active show' : '' }}" id="{{ $lang->iso_codes }}">
                                    <div class="card-body pb-2">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-10">
                                                <input type="text" class="form-control mb-1 gen_slug @error('name_'.$lang->iso_codes) is-invalid @enderror" lang="{{ $lang->iso_codes }}" 
                                                    name="name_{{ $lang->iso_codes }}" value="{{ old('name_'.$lang->iso_codes, $data['album']->fieldLang('name', $lang->iso_codes)) }}" placeholder="Enter name...">
                                            ...
                                            <div class="col-sm-10">
                                                <select class="selectpicker custom-select" name="category_id" data-style="btn-default">
                                                    ...
                                                    @foreach ($data['categories'] as $category)
                                                        <option value="{{ $category->id }}" {{ old('category_id', $data['album']->category_id) == $category->id ? 'selected' : '' }}>{!! $category->fieldLang('name') !!}</option>
                                                    @endforeach
                                                </select>
                                                @error('category_id')
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-10">
                                                <textarea class="form-control tiny-mce" name="description_{{ $lang->iso_codes }}">{!! old('description_'.$lang->iso_codes, $data['album']->fieldLang('description', $lang->iso_codes)) !!}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                </div>
            ...
            <div class="tab-pane fade" id="setting">
                <div class="card-body">
                        ...
                        <div class="col-sm-10">
                            <select class="selectpicker show-tick" name="publish" data-style="btn-default">
                                @foreach (config('custom.label.publish') as $key => $publish)
                                    <option value="{{ $key }}" {{ (old('publish', $data['album']->publish) == ''.$key.'') ? 'selected' : '' }}>
                                        {{ __($publish['title']) }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-10">
                            <label class="switcher switcher-success">
                                <input type="checkbox" class="switcher-input check-parent" name="is_detail" value="1" 
                                    {{ (old('is_detail', $data['album']->is_detail) == 1 ? 'checked' : '') }}>
                        ...
                        <div class="col-sm-10">
                            <div class="input-group">
                                <input type="text" class="form-control" id="image1" aria-label="Image" aria-describedby="button-image" name="image_file"
                                        value="{{ old('image_file', $data['album']->image_preview['file_path']) }}" placeholder="browse image..." readonly>
                            ...
                            <div class="row">
                                <div class="col-sm-6">
                                <input type="text" class="form-control" placeholder="title..." name="image_title" value="{{ old('image_title', $data['album']->image_preview['title']) }}">
                                </div>
                                <div class="col-sm-6">
                                <input type="text" class="form-control" placeholder="alt..." name="image_alt" value="{{ old('image_alt', $data['album']->image_preview['alt']) }}">
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-10">
                            <div class="input-group">
                                <input type="text" class="form-control" id="image2" aria-label="Image2" aria-describedby="button-image2" name="banner_file"
                                        value="{{ old('banner_file', $data['album']->banner['file_path']) }}" placeholder="browse banner..." readonly>
                                <div class="input-group-append" title="browse file">
                                    <span class="input-group-text">
                            ...
                            <div class="row">
                                <div class="col-sm-6">
                                <input type="text" class="form-control" placeholder="title..." name="banner_title" value="{{ old('banner_title', $data['album']->banner['title']) }}">
                                </div>
                                <div class="col-sm-6">
                                <input type="text" class="form-control" placeholder="alt..." name="banner_alt" value="{{ old('banner_alt', $data['album']->banner['alt']) }}">
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-10">
                            <select class="selectpicker show-tick" name="custom_view_id" data-style="btn-default">
                                ...
                                @foreach ($data['template'] as $custom)
                                    <option value="{{ $custom->id }}" {{ (old('custom_view_id', $data['album']->custom_view_id) == $custom->id) ? 'selected' : '' }}>
                                        {{ $custom->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="number" class="form-control" name="photo_limit" value="{{ old('photo_limit', $data['album']->photo_limit) }}" placeholder="for photo">
                        </div>
                    </div>
                </div>
            </div>

            @if (!empty($data['album']->field_category_id) || auth()->user()->hasRole('super'))
            <div class="tab-pane fade" id="field">
                <div class="card-body">
                        ...
                        <div class="col-sm-10">
                            <select id="field_category" class="selectpicker show-tick" name="field_category_id" data-style="btn-default">
                                ...
                                @foreach ($data['fields'] as $field)
                                    <option value="{{ $field->id }}" {{ $field->id == $data['album']->field_category_id ? 'selected' : '' }}>
                                        {{ $field->name }}
                                    </option>
                                @endforeach
                            </select>
                            @if (!empty($data['album']->field_category_id))    
                            <small>Current Field : <strong><i>{{ $data['album']->field->name }}</i></strong></small>
                            @endif
                        </div>
                    </div>
                    <div id="custom_field">
                    ...
                    @else
                    <input type="hidden" name="field_category_id" value="{{ $data['album']->field_category_id }}">
                    @endrole

                    @if (!empty($data['album']->field_category_id))
                    <div id="current_field">
                        @include('backend.master.fields.input-edit', ['module' => 'album'])
                    </div>
                    @endif
                </div>
            </div>
