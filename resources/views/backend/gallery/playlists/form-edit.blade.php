...
<div class="card">
    <div class="list-group list-group-flush account-settings-links flex-row">
        ...
        @if (!empty($data['fields']->count() > 0 || $data['playlist']->field_category_id) || auth()->user()->hasRole('super'))
        <a class="list-group-item list-group-item-action" data-toggle="list" href="#field">CUSTOM FIELD</a>
        @endif
    </div>
    <form action="{{ route('gallery.playlist.update', ['id' => $data['playlist']->id]) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="tab-content">
                                ...
                                <div class="tab-pane fade {{ $lang->iso_codes == config('custom.language.default') ? 'active show' : '' }}" id="{{ $lang->iso_codes }}">
                                    <div class="card-body pb-2">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-10">
                                                <input type="text" class="form-control mb-1 gen_slug @error('name_'.$lang->iso_codes) is-invalid @enderror" lang="{{ $lang->iso_codes }}" 
                                                    name="name_{{ $lang->iso_codes }}" value="{{ old('name_'.$lang->iso_codes, $data['playlist']->fieldLang('name', $lang->iso_codes)) }}" placeholder="Enter name...">
                                            ...
                                            <div class="col-sm-10">
                                                <div class="input-group">
                                                    <div class="input-group-prepend">
                                                        <span class="input-group-text"><strong>{{ url('/').'/video/{slugCategory}/' }}</strong></span>
                                                    </div>
                                                    <input type="text" class="form-control slug_spot @error('slug') is-invalid @enderror" lang="{{ $lang->iso_codes }}" name="slug"
                                                            value="{{ old('slug', $data['playlist']->slug) }}" placeholder="Enter slug...">
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-10">
                                                <textarea class="form-control tiny-mce" name="description_{{ $lang->iso_codes }}">{!! old('description_'.$lang->iso_codes, $data['playlist']->fieldLang('description', $lang->iso_codes)) !!}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                </div>
            ...
            <div class="tab-pane fade" id="setting">
                <div class="card-body">
                        ...
                        <div class="col-md-10">
                            <label class="switcher switcher-success">
                                <input type="checkbox" class="switcher-input check-parent" name="is_detail" value="1" 
                                    {{ (old('is_detail', $data['playlist']->is_detail) == 1 ? 'checked' : '') }}>
                        ...
                        <div class="col-sm-10">
                            <div class="input-group">
                                <input type="text" class="form-control" id="image1" aria-label="Image" aria-describedby="button-image" name="image_file"
                                        value="{{ old('image_file', $data['playlist']->image_preview['file_path']) }}" placeholder="browse image..." readonly>
                            ...
                            <div class="row">
                                <div class="col-sm-6">
                                <input type="text" class="form-control" placeholder="title..." name="image_title" value="{{ old('image_title', $data['playlist']->image_preview['title']) }}">
                                </div>
                                <div class="col-sm-6">
                                <input type="text" class="form-control" placeholder="alt..." name="image_alt" value="{{ old('image_alt', $data['playlist']->image_preview['alt']) }}">
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-10">
                            <div class="input-group">
                                <input type="text" class="form-control" id="image2" aria-label="Image2" aria-describedby="button-image2" name="banner_file"
                                        value="{{ old('banner_file', $data['playlist']->banner['file_path']) }}" placeholder="browse banner..." readonly>
                                <div class="input-group-append" title="browse file">
                                    <span class="input-group-text">
                            ...
                            <div class="row">
                                <div class="col-sm-6">
                                <input type="text" class="form-control" placeholder="title..." name="banner_title" value="{{ old('banner_title', $data['playlist']->banner['title']) }}">
                                </div>
                                <div class="col-sm-6">
                                <input type="text" class="form-control" placeholder="alt..." name="banner_alt" value="{{ old('banner_alt', $data['playlist']->banner['alt']) }}">
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-10">
                            <select class="selectpicker show-tick" name="custom_view_id" data-style="btn-default">
                                ...
                                @foreach ($data['template'] as $custom)
                                    <option value="{{ $custom->id }}" {{ (old('custom_view_id', $data['playlist']->custom_view_id) == $custom->id) ? 'selected' : '' }}>
                                        {{ $custom->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="form-group row">
                        <label class="col-form-label col-sm-2 text-sm-right">Video Limit <i></i></label>
                        <div class="col-sm-10">
                            <input type="number" class="form-control" name="video_limit" value="{{ old('video_limit', $data['playlist']->video_limit) }}" placeholder="for video">
                        </div>
                    </div>
                </div>
            </div>

            @if (!empty($data['playlist']->field_category_id) || auth()->user()->hasRole('super'))
            <div class="tab-pane fade" id="field">
                <div class="card-body">
                        ...
                        <div class="col-sm-10">
                            <select id="field_category" class="selectpicker show-tick" name="field_category_id" data-style="btn-default">
                                ...
                                @foreach ($data['fields'] as $field)
                                    <option value="{{ $field->id }}" {{ $field->id == $data['playlist']->field_category_id ? 'selected' : '' }}>
                                        {{ $field->name }}
                                    </option>
                                @endforeach
                            </select>
                            @if (!empty($data['playlist']->field_category_id))    
                            <small>Current Field : <strong><i>{{ $data['playlist']->field->name }}</i></strong></small>
                            @endif
                        </div>
                    </div>
                    <div id="custom_field">
                    ...
                    @else
                    <input type="hidden" name="field_category_id" value="{{ $data['playlist']->field_category_id }}">
                    @endrole

                    @if (!empty($data['playlist']->field_category_id))
                    <div id="current_field">
                        @include('backend.master.fields.input-edit', ['module' => 'playlist'])
                    </div>
                    @endif
                </div>
            </div>
